...
class DynamicCommunitySelection:
    ...
    def __init__(
        ...
        lvl = 0
        while str(lvl) not in self.levels and lvl <= max_level:
            lvl += 1

        # If we found a valid level, use it. Otherwise, default to level 0
        if str(lvl) in self.levels:
            self.starting_communities = self.levels[str(lvl)]
        else:
